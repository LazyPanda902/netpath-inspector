...
`netpath-inspector` uses a small `src/` package with two layers:
...
The source reference `GyulyVGC/sniffnet` influenced only the problem-space research. Implementation details are intentionally independent.
